...
def deconvolve(
    ...
    output: str = Option(  # noqa: B008
        None, help="Output location for deconvolved tomogram (.mrc/.rec format)", **PKWARGS
    ),
    pixel_size: float = Option(  # noqa: B008
        None,
        help="Input pixel size (optional). If not specified, it will be read from the tomogram's header. ATTENTION: This can lead to severe errors if the header pixel size is not correct.",
    ),
