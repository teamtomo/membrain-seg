...
# This function should only take label path as input, the generated skeleton will be saved in the same folder
def skeletonization(label_path: str):
    ...
    ----------
    label_path : str
        The path to the input file. This file should be a tomogram segmentation file in 
        either .nii, .nii.gz, or .mrc format. The function will perform checks to confirm 
        the file format.

    Returns
    -------
    None
        The function does not return any value. It saves the skeletonized image in the 
        same directory as the input file, replacing the original extension with '_skeleton.nii'.

    Raises
    ------
    ValueError
        If the input file format is not supported (.nii, .nii.gz, or .mrc), a ValueError is raised.

    Notes
    -----
    The skeletonization is based on the computation of the distance transform of the regions 
    labeled as '1' (foreground), followed by an eigenvalue analysis of the Hessian matrix of 
    the distance transform to identify ridge-like structures corresponding to the centerlines 
    ...
    # Segmentation consists of 0, 1 and 2 respectively for background, mask and labels to be ignored
    ...
    Skeleton = nonmaxsup(first_eigenvalue, first_eigenvector[:, :, :, 0], first_eigenvector[:, :, :, 1], first_eigenvector[:, :, :, 2], labels)
