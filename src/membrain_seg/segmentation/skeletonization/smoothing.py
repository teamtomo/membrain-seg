...
def process_hessian_tensors(hessian_components: list, kernel_size: int = 9, sigma: float = 1.0) -> list:
    """
    ...
    tensor_components = [torch.from_numpy(comp).float().to(device).unsqueeze(0).unsqueeze(0) for comp in hessian_components]
    
    filtered_tensors = [apply_gaussian_filter(tensor, kernel_size, sigma) for tensor in tensor_components]
